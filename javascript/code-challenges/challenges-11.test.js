...
const count = (target, input) => {
  // Solution code here...
  return input.flat().filter(i => i=== target).length;
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.flat().reduce((a,c) => a+c,0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let filtered = input.map(arr => arr.filter(i => typeof(i) === 'number'));
  let result = [];
  for (let i of filtered) {
    result.push(i.filter(x => x%5 === 0).map(x=> 2**x));
  }
  return result
};
...
let findShortest = (data) => {
  // Solution code here...
  let index = data.map(char => +char.height - +data[0].height)
  .indexOf(Math.min(...data.map(char => +char.height - +data[0].height)));
  //console.log(index)
