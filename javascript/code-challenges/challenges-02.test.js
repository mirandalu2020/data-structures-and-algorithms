...
const forLoopTwoToThe = (arr) => {
  ...
  let twoBaseArr = [];
  for (let int in arr) {
    twoBaseArr.push(Math.pow(2, arr[int]) );
  }
  return twoBaseArr;
};
...
const forEachTwoToThe = (arr) => {
  // Solution code here...
  let twoBaseArr = [];
  arr.forEach(int => twoBaseArr.push(Math.pow(2, int)));
  return twoBaseArr;
};
...
const extractStats = (arr) => {
  // Solution code here...
  //Expected [{"name": "speed", "total": 35}, {"name": "special-defense", "total": 112}, {"name": "special-attack", "total": 74}]
  let newArr = arr.map(
    (element)=>new Object({
